...
+ (NSInteger)maxProductOf:(NSInteger)numberOfItems itemsFromArray:(NSArray *)array {
    NSMutableArray *dig = [array mutableCopy];
    int j = 0;
    while (j < dig.count){
        if ([dig[j] isKindOfClass: [NSNumber class]]){
            j++;
        } else {
            [dig removeObject: dig[j]];
        }
    }
    NSArray *sorted = [dig sortedArrayUsingComparator:^(id a, id b) {
